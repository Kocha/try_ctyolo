...
///	int idx_i,idx_o;
	...
	int input_next_frame  = (4 + input_size_x +  input_h_padding) * (2*3 + input_size_y);// + 4;
	int output_next_frame = (4 + input_size_x + output_h_padding) * (2*3 + input_size_y);// + 4;
...
///					o_data = *(op + idx_o);
					idx_o1 = f * output_next_frame + ((y+2/*1*/) * output_next_line ) + x + 4;
					idx_o2 = f * output_next_frame + ((y+3/*2*/) * output_next_line ) + x + 4;
					_o_data1 = _mm_load_ps( op + idx_o1 );
					_o_data2 = _mm_load_ps( op + idx_o2 );

///					idx_i = n * (input_size_y * input_size_x) + ((y-1) * input_size_x) + x;
					idx_L0 = n * input_next_frame  + ((y+1/*0*/) * input_next_line ) + x + 4;
					idx_L1 = n * input_next_frame  + ((y+2/*1*/) * input_next_line ) + x + 4;
					idx_L2 = n * input_next_frame  + ((y+3/*2*/) * input_next_line ) + x + 4;
					idx_L3 = n * input_next_frame  + ((y+4/*3*/) * input_next_line ) + x + 4;
...
///						_o_data2 =  _mm_add_ps( _o_data2, _bias );
		...
		for(x=0;x<output_h_padding;x++) {
			for(y=0;y<input_size_y;y+=2) {
				idx_o1 = f * output_next_frame + ((y+2/*1*/) * output_next_line ) + x + input_size_x + 4;
				idx_o2 = f * output_next_frame + ((y+3/*2*/) * output_next_line ) + x + input_size_x + 4;
				*( op + idx_o1 ) = 0;
				*( op + idx_o2 ) = 0;
